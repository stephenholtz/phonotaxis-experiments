...
for i_rep = 1:num_reps
    ...
    for i_stim = 1:n_stimuli
        ...
        %curr_calib = calib(curr_stim_ind);
        
        %%%% MULTIPLY BY THE STIMULUS CALIBRATION %%%%%
        ...
        data_out = curr_stim.speaker_stimulus/(0.2+abs(max(curr_stim.speaker_stimulus)));

        % Make speaker objects
        Speaker = daq.createSession('directsound'); %#ok<*SAGROW>
        Speaker.UseStandardSampleRates = false; %#ok<*UNRCH>
        Speaker.Rate = speaker_rate;
        Speaker.addAudioOutputChannel(audio_ids{curr_stim.curr_speaker},1);
        disp(curr_stim.curr_speaker)
        if Speaker.Rate ~= speaker_rate
            error(['Speaker Rate is NOT ' num2str(speaker_rate) ', check configuration in Seraph 8+ settings'])
        end
        
        Speaker.queueOutputData(data_out');
        Speaker.startForeground();
